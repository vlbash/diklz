...
{
    public abstract class GeneratedClassXLS : GeneratedClassBase
    {
        private const double maxWidth = 7;
        private const double defColWidth = 10;

        public byte[] CreatePackageAsBytes(Object list)
        {
            using (var mstm = new MemoryStream())
            {
                using (SpreadsheetDocument package = SpreadsheetDocument.Create(mstm, SpreadsheetDocumentType.Workbook))
                {
        ...
        protected void CreateParts(SpreadsheetDocument document, Object list)
        {
         ...
         //   SetStyleSheet(workbookStylesPart);

            ThemePart themePart = workbookPart.AddNewPart<ThemePart>("rId2");
            GenerateThemePartContent(themePart);

            WorksheetPart worksheetPart = workbookPart.AddNewPart<WorksheetPart>("rId1");
            GenerateWorksheetPartContent(worksheetPart, list);

            SetPackageProperties(document);

        }

        protected void InitColumns(Columns columns, int collCount)
        {
            for (int i = 0; i < collCount; i++)
            {
                var coll = new Column() { Min = (UInt32)(i + 1), Max = (UInt32)(i + 1), Width = defColWidth, BestFit = true, CustomWidth = true };
                columns.Append(coll);
            }
        }

        protected double CalculateColWidth(int textLength)
        {
            //width = Truncate([{Number of Characters} * {Maximum Digit Width} + {5 pixel padding}]/{Maximum Digit Width}*256)/256
            double width = Math.Truncate((textLength * maxWidth + 20) / maxWidth * 256) / 256;
            return width;
        }

        protected void CollAutoFit(Worksheet worksheet1)
        {
            //var cols = worksheet1.GetFirstChild<SheetData>().Elements<Row>();
            var cols = worksheet1.GetFirstChild<Columns>().Elements<Column>();

            //AutoFit
            var cells = worksheet1.Descendants<Cell>().OrderByDescending(o => o.GetValueLength());
            var largeCells = cells.GroupBy(p => p.GetMyColumnName()).Select(d => new { Name = d.Key, KeyCol = d.FirstOrDefault() });//.Select(g => new { Name = g.Key, Count = g.Count() });

            foreach (var item in largeCells)
            {
                int index = ColumnLetterToColumnIndex(item.Name);
                double newWidth = CalculateColWidth(item.KeyCol.GetValueLength());
                if (newWidth > defColWidth)
                {
                    var currentCol = cols.Where(c => c.Min == index).FirstOrDefault();
                    if (currentCol != null)
                    {
        ...
        protected virtual void GenerateExtendedFilePropertiesPart1Content(ExtendedFilePropertiesPart extendedFilePropertiesPart1)
        {
        ...
        protected virtual void GenerateWorkbookPartContent(WorkbookPart workbookPart1)
        {
        ...
        protected virtual void GenerateWorkbookStylesPartContent(WorkbookStylesPart workbookStylesPart1)
        {
            ...
            CellFormat cellFormat5 = new CellFormat() { NumberFormatId = (UInt32Value)0U, FontId = (UInt32Value)0U, FillId = (UInt32Value)0U, BorderId = (UInt32Value)1U, FormatId = (UInt32Value)0U, ApplyBorder = true };
            CellFormat cellFormat6 = new CellFormat() { NumberFormatId = (UInt32Value)49U, FontId = (UInt32Value)0U, FillId = (UInt32Value)0U, BorderId = (UInt32Value)0U, FormatId = (UInt32Value)0U, ApplyNumberFormat = true };
        ...
        protected virtual void GenerateThemePartContent(ThemePart themePart1)
        {
        ...
        protected virtual void GenerateWorksheetPartContent(WorksheetPart worksheetPart, object list)
        {
        ...
        protected virtual void SetPackageProperties(OpenXmlPackage document)
        {
            document.PackageProperties.Creator = "";
            document.PackageProperties.Created = DateTime.Now;  //System.Xml.XmlConvert.ToDateTime("2017-12-27T12:51:53Z", System.Xml.XmlDateTimeSerializationMode.RoundtripKind);
            document.PackageProperties.Modified = DateTime.Now;  //System.Xml.XmlConvert.ToDateTime("2018-01-04T11:16:45Z", System.Xml.XmlDateTimeSerializationMode.RoundtripKind);
