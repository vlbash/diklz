...
{
    public sealed class OrgDepartment : BaseEntity, IOrgUnit
    {
        #region IDerivedEntity
        static IMapper _mapper = new MapperConfiguration(cfg => cfg.CreateMap<OrgDepartment, OrgUnit>()
            .ForMember(x => x.DerivedClass, opt => opt.MapFrom(o => typeof(OrgDepartment).Name)).MapOnlyIfChanged()).CreateMapper();
