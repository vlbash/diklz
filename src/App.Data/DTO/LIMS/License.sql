SELECT	
        ...
        lic.ISSUE_ORDER_DATE    as OrderDate,
		lic.ISSUE_ORDER_NUM     as OrderNumber,			-- № і дата наказу
		...
		FOR XML PATH(''), type) as varchar(max)), 2, 8000)) 
								as LicenseTypesName, 
        (substring(cast(
		(SELECT	'|' + CONVERT(varchar(10), llt.LICTYPE_ID) 
			FROM	LIC_LICENSE_LICTYPE llt			
		WHERE llt.DOC_ID	= lic.DOC_ID
		FOR XML PATH(''), type) as varchar(max)), 2, 8000)) 
								as LicenseTypesIds,
		...
		org.ADDRESS_IDX			as PostIndex,			-- Поштовий індекс
		org.PHONE				as Phone,		        -- Контактна інформація (тел., факс)
		org.PASSPORT			as SIDE_PASSPORT,		-- Паспортні дані (для фіз. особи)
		org.BANK_ACCOUNT		as SIDE_BANK_ACCOUNT,	-- Банківські реквізити 
		org.DIRECTOR_PIB		as OrgDirector,	        -- ПІБ керівника
		...
		CAST(otp.OPFG_TYPE_ID as varchar(10))as LegalFormType,
        otp.OPFG_NAME,                                  -- Орган?зац?йно-правова форма
        CAST(owt.OWNERSHIP_TYPE_ID as varchar(10))as OwnershipType,      
...
------------------------------------------------------------------------------------------------------------------------
	(   SELECT			
		...
		FOR XML PATH('BranchLicense'), TYPE)
...
AND		
   -- лицензия выбирается если во множественном выборе выбран хотя бы один вид деятельности соответствующий лицензии
		EXISTS (SELECT 1
				...
				AND CHARINDEX(','+CAST(lct.LICTYPE_ID as varchar)+',', ','+'@@LicTypes@@'+',') > 0
				)
AND org.EDRPOU = '@@EDRPOU@@'
AND lic.LICSTAT_ID = 1