$(document).ready(function () {
    $(document).on("change", "#LicenseType", function () {
        ...
        var msgId = $("#Id").val();
        var orgId = $('#OrgUnitId').val();
        $.ajax({
            url: "/MSG/MessageTypes/GetMpdByLicense/?mpd=" + $(licType).val() + "&msgType=" + msgType + "&msgId=" + msgId + '&orgId=' + orgId,
            type: "GET",
    ...
    $(document).on("change", "#OrgPositionTypeEnum", function () {
        var posType = this;
        var msgId = $("#Id").val();
        var orgId = $('#OrgUnitId').val();
        $.ajax({
            url: "/MSG/MessageTypes/GetAssigneeByLicense?msgId=" + msgId + "&orgPosType=" + $(posType).val() + '&orgId=' + orgId,
            type: "GET",
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                var mpd = "<option></option>";
