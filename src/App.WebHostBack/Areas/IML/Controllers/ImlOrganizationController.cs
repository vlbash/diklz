...
namespace App.HostBack.Areas.IML.Controllers
{
    [Area("IML")]
    [Authorize]
    public class ImlOrganizationController: CommonController<OrganizationExtListDTO, ImlOrganizationExtFullDTO, OrganizationExt>
    {
        private readonly IImlOrganizationService _service;
        private readonly IAtuAddressService _atuAddressService;

        public ImlOrganizationController(IConfiguration configuration, ISearchFilterSettingsService searchFilterSettingsService, IImlOrganizationService service, IAtuAddressService atuAddressService)
        ...
        public IActionResult ChooseOrganization(string returnUrl)
        {
            ...
            return View(new ImlOrganizationExtMediumDTO());
        }

        [HttpPost]
        public async Task<IActionResult> ChooseOrganization(string returnUrl, ImlOrganizationExtMediumDTO model)
        {
            if (string.IsNullOrEmpty(returnUrl))
            {
                return View($"{nameof(ChooseOrganization)}Error");
        ...
        {
            var result1 = DataService.GetDto<ImlOrganizationExtShortDTO>(
                    predicate: dto =>
        ...
        {
            ImlOrganizationExtFullDTO organization;

            if (paramList.TryGetValue("Id", out var idStr) && Guid.TryParse(idStr, out var id))
            {
                organization = DataService.GetDto<ImlOrganizationExtFullDTO>(dto => dto.Id == id).SingleOrDefault();
            }
            else
            {
        ...
        public async Task<IActionResult> EditSgd(Guid? id, string returnUrl)
        {
            ...
            ViewBag.returnUrl = returnUrl;
            return View(new ImlOrganizationExtFullDTO());
        }

        [HttpPost]
        public async Task<IActionResult> EditSgd(Guid id, string returnUrl, ImlOrganizationExtFullDTO model)
        {
            if (id != model.Id)
            {
                return NotFound();
        ...
        [HttpPost]
        public async Task<IActionResult> EditFop(Guid id, string returnUrl, ImlOrganizationExtFullDTO model)
        {
            if (id != model.Id)
            {
                return NotFound();
        ...
        [NonAction]
        public override async Task<IActionResult> Edit(Guid id, ImlOrganizationExtFullDTO model)
        {
            return await Task.Run(() => NotFound());
        }
