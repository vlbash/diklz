...
namespace App.HostBack.Areas.IML.Controllers
{
    ...
    public class ImlMedicineController: CommonController<ImlMedicineListDTO, ImlMedicineDetailDTO, ImlMedicine>
    {
        ...
        //додавання одного лікарського засоба
        //[HttpGet]
        //public async Task<IActionResult> DrugList(Guid? appId)
        //{
        //    if (appId == null)
        //        return NotFound();
        //    ViewBag.IsAddable = _entityStateHelper.ApplicationAddability(appId.Value).IsMedicine;
        //    var medicines = (await DataService.GetDtoAsync<ImlMedicineListDTO>(x => x.ApplicationId == appId)).ToList();
        //    medicines.ForEach(x => x.IsEditable = _entityStateHelper.IsEditableMedicine(x.Id) ?? false);
