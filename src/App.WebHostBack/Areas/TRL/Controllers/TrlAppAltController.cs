...
namespace App.HostBack.Areas.TRL.Controllers
{
    ...
    public class TrlAppAltController: CommonController<TrlAppDetailDTO, TrlAppDetailDTO, TrlApplication>
    {
        ...
        private TrlApplicationAltService _trlApplicationAltService;
        ...
        public async Task<IActionResult> CreateOnOpen(string sort, Guid? orgId)
        {
            ...
            try
            {
                appId = await _trlApplicationAltService.CreateOnOpen(sort, orgId, true);
            }
            catch (Exception)
            {
        ...
        public async Task<IActionResult> AltAppDetails(Guid id, string sort)
        {
            ...
            switch (sort)
            {
                ...
                case "AddBranchApplication":
                    HttpContext.ModifyCurrentBreadCrumb((crumb => crumb.Name = "Заява про внесення до ЄДР відомостей про місце провадження господарської діяльності - Додавання МПД(торгівля)"));
                    return View("Details_AddBranchApplication", application);
                case "RemBranchApplication":
                    HttpContext.ModifyCurrentBreadCrumb((crumb => crumb.Name = "Заява про внесення змін до ЄДР у зв’язку з припиненням діяльності за певним місцем провадження - Видалення МПД(торгівля)"));
        ...
        public async Task<IActionResult> SubmitApplication(Guid id)
        {
            if (id == Guid.Empty)
                return await Task.Run(() => NotFound());
            var app = _commonDataService.GetEntity<TrlApplication>(x => x.Id == id)?.FirstOrDefault();
            if (app == null)
                return await Task.Run(() => NotFound());
