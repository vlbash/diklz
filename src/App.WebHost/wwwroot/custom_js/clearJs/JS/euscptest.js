...
var URL_GET_CERTIFICATES = "/custom_js/clearJs/Data/CACertificates.p7b?version=1.0.17";
var URL_CAS = "/custom_js/clearJs/Data/CAs.json?version=1.0.17";
...
var EUSignCPTest = NewClass({
    ...
    },
    {
        ...
        signData: function (urlActionSave) {
            //var elements = $("#checkboxesForSign .checkbox-files-item .checkbox");
            //for (var i = 0; i < elements.length; i++) {
            //    var element = elements[i];
            //    if (element.checked == false) {
            //        alert("Для того, щоб відправити заяву треба підписати " +
            //            "всі прикріпленні документи, погодьтесь на підписання всіх документів або " +
            //            "замініть невірні файли");
            //        return;
            //    }
            //}
            var signedFileModel = {};
                    ...
                    try {
                        ...
                        if (item.id == '00000000-0000-0000-0000-000000000000') {
                            if (item.isSystemFile)
                                item.file = '@@startFile@@' + item.file + "@@endFile@@";
                            else {
                        ...
                        obj.file = sign;
                        //ТУТ прогресс бар сколько файлов отработало и сколько осталось
...
function createNewCheckbox() {
    var html = '';
    FileModel.files.forEach(function (item, i, files) {
        html += '<div class="checkbox-files-item">';
        ...
        //checkbox.id = i;
        var checkbox = '<input type="checkbox" class="checkbox" name="' + item.name + '" id="' + i + '">';
        ...
        var label = '<label for="' + i + '">' + item.name + '</label>';

        html += checkbox + label + '</div>';

    });
    var container = document.getElementById('checkboxesForSign');
