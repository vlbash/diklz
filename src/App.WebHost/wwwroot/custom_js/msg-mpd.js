$(document).ready(function () {
    $(document).on("change", "#LicenseType", function () {
        ...
        var msgId = $("#Id").val();
        $.ajax({
            url: "/MSG/MessageTypes/GetMpdByLicense?mpd=" + $(licType).val() + "&msgType=" + msgType + "&msgId=" + msgId,
            type: "GET",
    ...
    $(document).on("change", "#OrgPositionTypeEnum", function () {
        ...
        var msgId = $("#Id").val();
        $.ajax({
            url: "/MSG/MessageTypes/GetAssigneeByLicense?msgId=" + msgId + "&orgPosType=" + $(posType).val(),
            type: "GET",
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                var mpd = "<option></option>";
