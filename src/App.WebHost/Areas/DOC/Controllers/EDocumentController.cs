...
/*
 * TODO: Нужно написать сервис для Edocument
 */
...
namespace App.Host.Areas.DOC.Controllers
{
    ...
    public class EDocumentController: BaseController<EDocumentListDTO, EDocumentDetailsDTO, EDocument>
    {
        ...
        public async Task<IActionResult> DetailsMsg(Guid? id, Guid msgId)
        {
            ...
            var branchContractors = _dataService.GetEntity<BranchEDocument>()
                .Where(x => x.EDocumentId == model.Id).ToList();
            var branchList = (await _dataService.GetDtoAsync<BranchListMsgDTO>(x => branchContractors.Select(y => y.BranchId)
                .Contains(x.Id))).ToList();
        ...
        public async Task<IActionResult> ListMsg(Guid appId, Guid msgId)
        {
            ...
            var branchMsgList = (await _dataService.GetDtoAsync<BranchListMsgDTO>(x => x.MessageId == msgId)).ToList();
        ...
        public async Task<IActionResult> EditMsg(Guid id, Guid msgId)
        {
            ...
            var branchIds = _dataService.GetEntity<BranchEDocument>().Where(x => x.EDocumentId == id).Select(x => x.BranchId);
            eDocument.ListOfBranches = branchIds.ToList();
            var branchList = (await _dataService.GetDtoAsync<BranchListMsgDTO>(x => x.MessageId == msgId)).ToList();

            var multiSelectList = new MultiSelectList(branchList, nameof(BranchListMsgDTO.Id), nameof(BranchListMsgDTO.Name));
            ViewBag.multiSelectBranchList = multiSelectList;
        ...
        public override async Task<IActionResult> Edit(Guid id, EDocumentDetailsDTO model)
        {
            ...
            try
            {
                ...
                var branchEDocuments =
                    _dataService.GetEntity<BranchEDocument>().Where(x => x.EDocumentId == model.Id).ToList();

                if (branchEDocuments.Count > 0)
                {
                    foreach (var x in branchEDocuments)
                    {
                        var temp = _dataService.GetEntity<Branch>().FirstOrDefault(e => e.Id == x.BranchId);

                        if (temp.IsFromLicense != true)
                        {
                            _dataService.Remove(x);
                        }
                    }
                }
            ...
            {
                case "PRL":
                    return RedirectToAction("Details", "PrlApp", new
                    {
                        Area = "PRL",
                        Id = model.ApplicationId
                    }, "eDocument");
                case "IML":
                    return RedirectToAction("Details", "ImlApp", new
                    {
                        Area = "IML",
                        Id = model.ApplicationId
                    }, "eDocument");
                case "TRL":
                    return RedirectToAction("Details", "TrlApp", new
                    {
                        Area = "TRL",
                        Id = model.ApplicationId
                    }, "eDocument");
        ...
        public async Task<IActionResult> EditMsg(Guid id, EDocumentDetailsMsgDTO model)
        {
            ...
            if (!model.ListOfBranches?.Any() ?? true)
            {
                var branchIds = _dataService.GetEntity<BranchEDocument>().Where(x => x.EDocumentId == id).Select(x => x.BranchId);
                model.ListOfBranches = branchIds.ToList();
                var branchList = await _dataService.GetDtoAsync<BranchListMsgDTO>(x => x.MessageId == model.MessageId);
                var multiselectlist = new MultiSelectList(branchList, nameof(BranchListMsgDTO.Id), nameof(BranchListMsgDTO.Name));
                ViewBag.multiSelectBranchList = multiselectlist;
                ModelState.Clear();
        ...
        public async Task<IActionResult> EditLicense(Guid id, EDocumentDetailsDTO model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var contractorBranches = _dataService.GetEntity<BranchEDocument>(x => x.EDocumentId == model.Id).ToList();
