...
{
    public class PrlApplicationAltService: IPrlApplicationAltService
    {
        private readonly ICommonDataService _commonDataService;
        private readonly IPrlLicenseService _prlLicenseService;
        private readonly IObjectMapper _objectMapper;

        public PrlApplicationAltService(ICommonDataService commonDataService, IPrlLicenseService prlLicenseService,
        ...
        {
            var license = _commonDataService.GetEntity<PrlLicense>(lic => lic.Id == licenseId).FirstOrDefault();
        ...
        public async Task<Guid> ProcessApplicationToApplication(Guid appId, Guid? licenseId = null, string sort = "", bool isBackOffice = false)
        {
            ...
            var licenseApplication = _commonDataService.GetEntity<PrlApplication>(x => x.Id == appId)
                .FirstOrDefault();
            ...
            var newApplication = new PrlApplication();
            ...
            var applicationContractorBranches =
                _commonDataService.GetEntity<PrlBranchContractor>(x => applicationBranchesIds.Contains(x.BranchId));
            var applicaitonContractorIds =
                applicationContractorBranches.Select(x => x.ContractorId).Distinct().ToList();
            var applicationContractors = _commonDataService
            ...
            foreach (var applicationContractor in applicationContractors)
            {
                ...
                licContractor.IsFromLicense = true;
            ...
            licenseContractors.ForEach(contractor => _commonDataService.Add(contractor, false));
            licenseContractorBranches.ForEach(contractorBr => _commonDataService.Add(contractorBr, false));
        ...
        public async Task Delete(Guid id, bool softDeleting)
        {
            try
            {
                var application = _commonDataService.GetEntity<PrlApplication>(x => x.Id == id).SingleOrDefault();
                ...
                var contractorBranches = _commonDataService
                ...
                var contractor = _commonDataService
